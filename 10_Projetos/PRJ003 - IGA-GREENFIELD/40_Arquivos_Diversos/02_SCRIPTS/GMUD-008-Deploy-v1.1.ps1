...
if (($ping_test | Out-String) -notmatch "received") {
...
$hub_test = ssh "$VM_USER@$VM_IP" "curl -sI https://registry-1.docker.io/v2/ 2>&1 | head -1" 2>&1
...
# Alteração na linha abaixo para garantir que só limpe se houver IDs reais
...
# ====================================================================
# STEP 4: CRIAÇÃO DO docker-compose.yml (MÉTODO ROBUSTO POR PIPE)
# ====================================================================
Write-Host "[STEP 4] Criando docker-compose.yml..." -ForegroundColor Yellow

# 1. Definimos o conteúdo do YAML em uma variável local do PowerShell
$dockerComposeYaml = @"
services:
  postgres:
...
# 2. Enviamos o conteúdo via pipe diretamente para o comando 'cat' na VM
# Isso elimina qualquer erro de 'Heredoc' ou 'COMPOSEEOF'
$dockerComposeYaml | ssh "$VM_USER@$VM_IP" "cat > /srv/prj003/docker-compose.yml"

# 3. Validar sintaxe
Write-Host "  Validando sintaxe do docker-compose.yml..." -ForegroundColor Gray
$compose_validation = ssh "$VM_USER@$VM_IP" "cd /srv/prj003 && docker compose config" 2>&1
