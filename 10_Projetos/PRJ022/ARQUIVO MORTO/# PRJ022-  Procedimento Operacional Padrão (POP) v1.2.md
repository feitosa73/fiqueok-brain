...
**Versão:** 1.2  
**Data:** Maio/2026  
**Status:** ✅ **COMPLETO** — Baseado na execução da POC e validação em laboratório  
...
Versão: 1.2 (correções de segurança e rede)
"""
...
**Por que?** Este campo será a **chave de correlação** no próximo passo.
...
| Campo                    | Valor                                                                        |
| ------------------------ | ---------------------------------------------------------------------------- |
| **Rule name**            | `Correlacao_Matricula`                                                       |
| **Description**          | `Regra para correlacionar via employee_id do CSV com personalNumber do User` |
| **Enabled**              | `True` (NÃO deixar `Undefined`)                                              |
| **Weight**               | (deixar em branco)                                                           |
| **Tier**                 | (deixar como está)                                                           |
| **Ignore if matched by** | (deixar `Undefined`)                                                         |

### 12.2. Configurar o correlator (o par de atributos)

1. Clique no ícone de edição (lápis) ou no link da regra
2. Clique em **Add correlator**
...
4. Clique em **Confirm settings** ou **Save**

### 12.3. Salvar a Correlation

1. Clique no botão verde **Save correlation settings**
2. Clique em **Exit wizard** para voltar à tela principal do wizard de 8 blocos
...
| **Tarefa não cria usuários** | Correlation ou Synchronization mal configuradas | Verificar `UNMATCHED` → `addFocus` |
| **Usuários duplicados** | Correlation não está funcionando | Verificar regra: `employee_id` → `personalNumber` |
...
| L08 | O mapeamento `employee_id` → `personalNumber` serve como chave de correlação |
| L09 | O botão "Save & Run" na tarefa salva **e** executa imediatamente |

### 18.2. Frameworks de Compliance Aplicados
...
*PRJ022 — Procedimento Operacional Padrão (POP) v1.2*  
*Living Lab Fiqueok*  
*Arquivado em: `FiqueokBrain/PRJ022/POP-PRJ022-A-CSV-Integration-v1.2.md`*
