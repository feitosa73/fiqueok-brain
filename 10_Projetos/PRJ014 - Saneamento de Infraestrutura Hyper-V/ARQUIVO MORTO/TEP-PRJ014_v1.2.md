...
| ------------------- | ------------------------------------------------------------- |
| **Código**          | TEP-PRJ014                                                    |
| **Versão**          | 1.2 — Adendo pós-encerramento (01/04/2026)                    |
| **Versão Anterior** | 1.1 — Adendo pós-encerramento (29/03/2026)                    |
...
| **1.2** | **01/04/2026** | **Paulo Feitosa Lima** | **Adendo: segunda reprovação do Win2022-GF-GEN2.vhdx — inapto para AD DS por metadados residuais irremovíveis (DCPromo.General.54). Adição de L21. Atualização de P4 para prioridade Média.** |
...
## 3. Contexto do Adendo v1.2

Em 01/04/2026, durante a tentativa de uso do `Win2022-GF-GEN2.vhdx` para provisionar um novo Controlador de Domínio (domínio `acesstage.local`), foi identificada uma **terceira não-conformidade** neste ativo, independente das duas anteriores:

| Não-Conformidade | Impacto | Decisão |
|------------------|---------|---------|
| Metadados de AD DS irremovíveis — VHDX clonado de `ID-P-01` (ex-DC `corp.fiqueok.com.br`) carrega estado profundo de AD DS que persiste mesmo após limpeza manual de registro, SYSVOL e NTDS | Falha persistente `DCPromo.General.54` em qualquer tentativa de promoção a DC, independentemente do domínio-alvo | GD classificado como **inapto para AD DS** |

**Métodos de limpeza tentados e confirmados como insuficientes:**

| Ação | Resultado |
|------|-----------|
| Remoção de chaves de registro (`Domain`, `NV Domain`) | Insuficiente |
| Limpeza de sufixo DNS | Insuficiente |
| Reconfiguração do WinRM | Insuficiente |
| Configuração de IP estático | Insuficiente |
| Limpeza e recriação de `C:\Windows\NTDS` e `C:\Windows\SYSVOL` | Insuficiente |
| Teste com domínio diferente (`lab.local`) | Mesmo erro |

> 📎 Referência completa: Troubleshooting — Falha na Promoção de DC com Golden Disk GEN2 (01/04/2026)

---

## 4. Ciclo de Purificação — Intervenção de 29/03/2026
...
## 5. Inventário Final de Golden Disks — Estado Verificado

### 5.1. Windows Server 2022

| Arquivo | Tamanho | ReadOnly | Status | Observação |
|---------|---------|----------|--------|------------|
| Win2022-GF-GEN1.vhdx | 10.29 GB | ✅ True | Legado / Ativo | GEN1; base para DCs sem Secure Boot; **apto para AD DS** |
| Win2022-GF-GEN2.vhdx | 13.82 GB | ✅ True | ❌ Reprovado / Arquivado | ParentPath detectado (v1.1) + **metadados AD DS irremovíveis (v1.2)**; não usar para novos provisionamentos; **inapto para AD DS** |
...
## 6. Inventário de VMs — Estado Final
...
## 7. Lições Aprendidas — Atualização (v1.2)
...
| **L21** | **Golden Disks clonados de servidores que foram DC carregam metadados de AD DS irremovíveis por limpeza manual. O erro `DCPromo.General.54` é o sintoma diagnóstico. Limpeza de registro, SYSVOL e NTDS é insuficiente. A solução é usar GD originado de instalação limpa.** | **Win2022-GF-GEN2.vhdx inapto para AD DS — validado em 01/04/2026** | **Nunca usar como base para DC um GD clonado de servidor com histórico de AD DS. Pre-Flight deve incluir CHECK de resquícios AD DS (ver POP-GOLDEN-DISK-001 v1.1, Seção 4, CHECK 4).** |

---

## 8. Pendências Identificadas (Não Bloqueantes)
...
| **P4** | **Win2022-GF-GEN2.vhdx (reprovado) ainda em disco** | **Média** *(elevada de Baixa em v1.2)* | **Duas reprovações acumuladas: ParentPath (v1.1) + metadados AD DS irremovíveis (v1.2). Recomendado descarte para liberar 13.82 GB. Manter apenas se houver justificativa de arquivo histórico documentada.** |
...
|--------|------|------|--------|
| GRC Lead / Responsável | Paulo Feitosa Lima | 29/03/2026 | ✅ APROVADO (v1.0 e v1.1) |
| GRC Lead / Revisor | Paulo Feitosa Lima | 01/04/2026 | ✅ APROVADO (v1.2) |
| GRC Advisor | Claude (Anthropic) | 01/04/2026 | ✅ REVISADO (v1.2) |

---

**FIM DO TEP-PRJ014 v1.2**
