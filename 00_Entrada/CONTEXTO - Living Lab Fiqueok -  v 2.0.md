...
## Documento de Referência para RAG — v2.0

**Gerado em:** Abril de 2026  
**Responsável:** Paulo Feitosa (IAM Specialist / Auditor / GRC Lead)  
**Fonte:** Evidência primária — vaults Obsidian exportados (PRJ001 a PRJ008)  
**Uso:** Contexto para AnythingLLM + Ollama / DeepSeek-R1  
**Versão anterior:** v1.0 (PRJ001–PRJ007)  
**Esta versão adiciona:** PRJ008 (Shadow API REST), lições do bloqueio de conector REST, atualização de riscos e inventário
...
5. Linha do Tempo da Saga (PRJ001–PRJ008)
...
13. PRJ008 — Shadow API REST (Integração IGA) ← **NOVO**
    
14. Lições Aprendidas Transversais
    
15. Frameworks de Conformidade Adotados
    
16. Inventário de Ativos e Topologia de Rede
    
17. Governança e Gestão de Decisões
    
18. Papel das IAs no Laboratório
    
19. Riscos Abertos e Pendências Futuras
    
20. Glossário Técnico do Laboratório
...
**Status:** 🟡 Aceitação Parcial — Shadow API certificada; integração midPoint via DatabaseTable Connector; conector REST nativo indisponível

### Contexto e Justificativa — Por que o PRJ008 foi criado

O PRJ008 nasceu como consequência direta de duas lições fundamentais dos projetos anteriores:

1. **PRJ006 (JDBC Abortado):** A tentativa de integrar o midPoint diretamente ao banco MariaDB do OrangeHRM via JDBC foi abortada por representar um **anti-padrão arquitetural**. O schema altamente normalizado do OrangeHRM exigia queries com 8+ JOINs, duplicava lógica de negócio em SQL e criava acoplamento insustentável com o schema do sistema de terceiros.
    
2. **PRJ003 (Princípio API-first):** O PRJ003 consolidou o princípio arquitetural de que **sistemas devem se integrar via APIs**, respeitando a camada de aplicação, não acessando bancos de dados diretamente.
    

**A decisão estratégica foi:** construir uma Shadow API REST que atuasse como camada de abstração entre o OrangeHRM e o midPoint, normalizando os dados (UTF-8 NFC), aplicando controles de segurança (OAuth 2.0, API Key, Vault) e expondo um contrato estável independente do schema interno do OrangeHRM.

### O que foi entregue e certificado (ARQ-PRJ008-Shadow-API-v2.1-FINAL — 17/04/2026)

A Shadow API foi **completamente implementada, testada e certificada** como produção-ready:

**Infraestrutura:**

- VM `api-gf-01` (Ubuntu 24.04, Tailscale IP `xxx.xxx.xxx.xxx`)
    
- FastAPI + Uvicorn rodando na porta 8000
    
- Serviço systemd `shadow-api.service` com `Restart=always` e habilitado no boot
    
- Token Vault em `/var/lib/shadow-api/vault_token` (chmod 600, chown paulo:paulo)
    
- Cron de renovação do token diariamente às 00:00
    

**Segurança:**

- Autenticação via header `X-API-KEY: Fiqueok-Security-Token-2026`
    
- Política Vault `api-proxy-policy` com `read` apenas em `secret/data/orangehrm/*`
    
- Usuário MariaDB `svc_shadow_api` com `SELECT` apenas em `hs_hr_employee` e `ohrm_user`
    
- Tailscale ACLs com `tag:consultor` (Daniel) restrito a portas 80, 8000, 8085
...
- `GET /employees` → Retorna lista de colaboradores via SQLAlchemy (parametrizado, seguro)
    
- Normalização UTF-8 NFC e omissão de campos nulos (`response_model_exclude_none=True`)
    
- Logs estruturados no `journald` com correlation ID
    

**Evidências de certificação:**

- ARQ-PRJ008-Shadow-API-v2.1-FINAL.md — documentação AS-BUILT
    
- Testes de validação documentados (Seção 8 do ARQ)
    
- Checklist de validação operacional (script bash)
    

### Por que o PRJ008 ainda não foi finalizado — o bloqueio do conector REST

O objetivo final do PRJ008 era que o **midPoint consumisse a Shadow API via conector REST nativo**. Durante a Sprint 6 (integração), identificou-se que:

**Problema 1 — `connector-rest` Polygon não existe como JAR standalone:**

- A comunidade Evolveum mantém o `connector-rest` como uma **biblioteca Maven** (dependência para desenvolvedores), não como um conector ICF deployável.
    
- URLs testadas (Nexus Evolveum, Maven Central) retornaram HTTP 404 para qualquer tentativa de download direto do JAR.
    

**Problema 2 — ScriptedREST 1.1.1.e2 é incompatível com Java 21 (midPoint 4.10):**

- O conector ScriptedREST (ForgeRock/Evolveum) foi baixado e injetado no diretório `connid-connectors/`
    
- O midPoint reconheceu o conector (OID `c89f121b-f6a3-483e-9300-91b91bbe06f5`)
    
- **Test Connection falhou com erro:** `groovy/util/slurpersupport/GPathResult`
    
- Causa raiz: ScriptedREST foi desenvolvido para Groovy 2.x / Java 11. O midPoint 4.10 roda Java 21, onde `GPathResult` foi movido de pacote.
    
- O erro ocorre na **inicialização do bundle do conector**, antes de qualquer script Groovy ser executado.
    

**Workaround implementado — DatabaseTable Connector:**

- O midPoint foi integrado ao MariaDB do OrangeHRM via **DatabaseTable Connector nativo**
    
- Driver MySQL (`mysql-connector-j-8.0.33.jar`) foi adicionado ao classpath
    
- O Resource `OrangeHRM via DatabaseTable Connector` está funcional e sincronizando dados
    
- **Circunstancialmente,** o midPoint está lendo o banco diretamente — o oposto do que o PRJ008 foi criado para evitar. Mas é o único caminho viável sem um conector REST funcional.
    

### Estado atual da integração midPoint ↔ Shadow API

|Componente|Status|Detalhe|
|---|---|---|
|Shadow API (`api-gf-01:8000`)|✅ Certificada|FastAPI, Vault, UTF-8 NFC, systemd|
|Resource REST no midPoint|❌ Não funcional|Conector REST indisponível para Java 21|
|Resource DatabaseTable no midPoint|✅ Funcional|Integração direta ao MariaDB como workaround|
|Sincronização JML|✅ Possível via DatabaseTable|O ciclo de identidade funciona, mas viola o princípio API-first|
|Shadow API como "plano de retomada"|🟡 Aguardando|Quando houver conector REST compatível, substitui o DatabaseTable|

### Decisão de Aceitação Parcial (TEP-PRJ008-v1.0-FREEZING)

O projeto foi formalmente **congelado (FROZEN)** em 14/04/2026 por bloqueio técnico de infraestrutura, com aceitação parcial dos entregáveis:

**Aceito (Shadow API certificada):**

- ✅ Código da Shadow API (FastAPI + Vault + SQLAlchemy)
    
- ✅ Documentação AS-BUILT (ARQ-PRJ008-Shadow-API-v2.1-FINAL)
    
- ✅ Infraestrutura de produção (systemd, cron, ACLs)
    
- ✅ Segredos gerenciados via Vault
    

**Pendente (integração nativa):**

- ❌ Resource REST no midPoint consumindo a Shadow API
    
- ❌ Validação JML end-to-end via Shadow API (rota REST)
    

**Retomada condicionada a:**

- Disponibilidade de um conector REST compatível com midPoint 4.10 / Java 21
    
- Build customizado do `connector-rest` via Maven (Caminho A no TEP)
    
- Release oficial da Evolveum com suporte a Java 21 (Caminho B)
    

### Lições do PRJ008 incorporadas ao laboratório

- **Conector REST não é garantido em versões recentes do midPoint:** A documentação da Evolveum sugere que o Polygon REST Connector é uma alternativa, mas não há JAR standalone. A distinção entre "biblioteca para desenvolvedor" e "conector deployável" não é clara.
    
- **ScriptedREST é legado e incompatível com Java 21:** A última versão disponível (1.1.1.e2) é de 2021, anterior ao Java 17/21. O erro `GPathResult` é um sintoma de incompatibilidade de runtime que não será corrigido sem rebuild.
    
- **Workaround não é solução:** O DatabaseTable Connector funciona, mas viola o princípio API-first que motivou o PRJ008. Ele é aceitável como contingência, mas não como arquitetura final.
    
- **TEP de freezing é documento de governança, não fracasso:** O PRJ008 entregou 90% do escopo. O bloqueio está fora do controle do laboratório. O TEP documenta a decisão de parar e aguardar, não de abandonar.
...
**Conector REST não é garantido em versões recentes do midPoint** (PRJ008): midPoint 4.10/Java 21 não possui conector REST nativo funcional. `connector-rest` Polygon é biblioteca Maven, não JAR standalone. ScriptedREST 1.1.1.e2 é incompatível com Java 21 (erro `GPathResult`). Workaround necessário: DatabaseTable Connector ou build customizado.
...
Abortar projeto por razões certas é decisão de governança mais valiosa que entregar solução frágil. O PRJ006 falhou funcionalmente mas entregou conhecimento, infraestrutura saneada e decisão arquitetural estratégica documentada. O PRJ008 foi congelado (TEP de freezing) por bloqueio externo — não por falha de execução — e isso é documentado como governança correta. "Fail fast" documenting everything é maturidade de processo.
...
|iga-gf-01|iga-gf-01|midPoint 4.10 + PostgreSQL (original)|192.168.70.10|Ativo via mesh|✅ Ativo|
|**iga-gf-02**|**iga-gf-02**|**midPoint 4.10** (instalação limpa, abr/2026)|192.168.111.153|**xxx.xxx.xxx.xxx**|✅ **Ativo (nova VM)**|
|**api-gf-01**|**api-gf-01**|**Shadow API FastAPI** (PRJ008)|192.168.99.64|**xxx.xxx.xxx.xxx**|✅ **Ativo**|
|ID-P-01|id-p-01|AD DS ([corp.fiqueok.com.br](https://corp.fiqueok.com.br/))|xxx.xxx.xxx.xxx|—|✅ Ativo|
...
- cloudflared via systemd (túnel para [vault.fiqueok.com.br](https://vault.fiqueok.com.br/))
...
- Bind mounts em `/srv/prj003/` (iga-gf-01) ou `/srv/iga-project/` (iga-gf-02)
    

**api-gf-01 (PRJ008):**

- Shadow API FastAPI via uvicorn + systemd (porta 8000)
    
- Tailscale via systemd (IP fixo xxx.xxx.xxx.xxx)
    
- Token Vault em `/var/lib/shadow-api/vault_token`
...
|**DeepSeek**|Validação de infraestrutura|Análise de logs de container, validação de conectividade, troubleshooting de midPoint|
...
|**[corp.fiqueok.com.br](https://corp.fiqueok.com.br/)**|Domínio Active Directory do laboratório, gerenciado por ID-P-01 (xxx.xxx.xxx.xxx)|
...
_CONTEXTO_LivingLab_Fiqueok_v2.0.md — Documento de Referência para RAG_  
_Baseado exclusivamente em evidência primária dos vaults Obsidian exportados_  
*Cobrindo PRJ001 a PRJ008 — Dezembro/2025 a Abril/2026*  
_Paulo Feitosa — Living Lab Fiqueok_  
_Gerado com Claude Sonnet como GRC Lead_
