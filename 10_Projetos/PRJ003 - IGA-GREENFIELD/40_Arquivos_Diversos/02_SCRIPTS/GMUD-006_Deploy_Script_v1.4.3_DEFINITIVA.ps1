...
# Versão: 1.4.3 DEFINITIVA (Correção de Parser PowerShell)
# ==============================================================================

# 0. BANNER DE IDENTIFICAÇÃO
Write-Host @"
╔══════════════════════════════════════════════════════════════════════════╗
║                        GMUD-006 v1.4.3 DEFINITIVA                        ║
║           Deploy IGA com Orquestração de Bootstrap                       ║
║                   PRJ003 - IGA Greenfield                                ║
║         (Correção de Parser + Senha Administrator)                       ║
╚══════════════════════════════════════════════════════════════════════════╝
...
ssh $VM_USER@$VM_IP "sudo rm -rf $BASE/data/postgres/* $BASE/data/midpoint/var/* $BASE/logs/midpoint/* && sudo mkdir -p $BASE/data/postgres $BASE/data/midpoint/var $BASE/logs/midpoint $BASE/evidencias && sudo chown -R ${VM_USER}:${VM_USER} $BASE"
...
# CORREÇÃO CRÍTICA v1.4.3: Remoção de backticks incorretos que causavam erro de parser
$composeContent = @"
services:
  postgres:
...
# ---------------------------------------
# CORREÇÃO CRÍTICA v1.4.3: Lógica refatorada sem backticks incorretos
...
Write-Host "  1. 'Initial password for administrator set from environment'" -ForegroundColor Green
Write-Host "  2. 'Created User:administrator'" -ForegroundColor Green
Write-Host "  3. 'Server startup in XXXXX milliseconds'" -ForegroundColor Green
...
Write-Host "  [ ] V01: Containers em execução" -ForegroundColor Gray
...
# ==============================================================================
# FIM DO SCRIPT GMUD-006 v1.4.3 DEFINITIVA
# ==============================================================================
