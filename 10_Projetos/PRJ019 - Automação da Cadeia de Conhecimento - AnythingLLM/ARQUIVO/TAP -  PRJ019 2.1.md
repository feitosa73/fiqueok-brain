...
|**Integração Vault**|AppRole authentication, leitura da API Key do AnythingLLM em `secret/data/anythingllm` via IP da Tailscale (xxx.xxx.xxx.xxx)|
...
|**Containerização**|Docker Desktop (Windows Host) + Bind Mounts|Portabilidade, isolamento, respeita CONSTRAINT-001, sem Hyper-V VM|
|**Logs**|JSON + rotação (`logrotate`)|Rastreabilidade para ISO 27001 A.12.4|

### 3.1. Fluxo de Execução

1. Watcher detecta novo arquivo em `10_Projetos/PRJXXX/` via Bind Mount
2. API Service recebe evento via callback
3. Autentica no Vault (AppRole) → obtém API Key do AnythingLLM (via Tailscale IP xxx.xxx.xxx.xxx)
4. Classificador determina a camada (1 a 6)
5. Chama POST /workspace/{slug}/documents no AnythingLLM Desktop via `host.docker.internal:3001`
...
### 4.1. Infraestrutura (Atualizada para v2.1)
...
|---|---|---|---|
|**Vault AppRole**|PRJ007|🔴 Pendente|Criar política `policy-svc-ingestor` e RoleID/SecretID (acesso via Tailscale xxx.xxx.xxx.xxx)|
...
|**PAD-002 reforçado**|Vault AppRole via Tailscale IP xxx.xxx.xxx.xxx|Implementação|
...
|**Fase 2**|Desenvolver API Service + Watcher (Docker Bind Mounts)|2 dias|Fase 1|
|**Fase 3**|Implementar classificador por camadas|0.5 dia|Fase 2|
|**Fase 4**|Testes de integração (watcher → Vault → AnythingLLM via `host.docker.internal`)|1 dia|Fase 3|
...
|E03|Dockerized Watcher Service (Python Watchdog)|Contêiner Docker + Bind Mounts|VM do PRJ019 (ou Windows Host)|
...
- Vault autentica via AppRole sem credenciais hardcoded no código (comunicação via Tailscale)
...
**FIM DO TERMO DE ABERTURA DO PROJETO — PRJ019 v2.1**

📄 **Documento salvo como:** `TAP-PRJ019-v2.1.md`  
