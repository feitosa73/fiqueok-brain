...
**Versão:** 1.3  
**Data:** Maio/2026  
**Status:** ✅ **VALIDADO** — Baseado na execução da POC e nas lições do PRJ004  
...
12. [Passo 6: Configuração da Correlation — ESTRATÉGIA VALIDADA (PRJ004)](#12-passo-6-configuração-da-correlation--estratégia-validada-prj004)
...
### 3.9. Decisão Arquitetural Crítica: Por que NÃO configuramos Correlation Explícita

Após extensa investigação técnica e múltiplas tentativas frustradas, constatou-se que o **CsvConnector v2.9 no midPoint 4.10 apresenta limitações no parser de XML para o bloco `<correlation>`** quando configurado explicitamente.

**As tentativas realizadas e seus respectivos erros:**

| # | Tentativa | Sintaxe utilizada | Erro | Resultado |
|---|-----------|-------------------|------|-----------|
| 1 | Via GUI (sem XML) | Apenas `<ref>personalNumber</ref>` | `No correlation items` | ❌ Falhou |
| 2 | Adicionar `<source>` com `ri:employee_id` | `<source><path>ri:employee_id</path></source>` + `<target><path>personalNumber</path></target>` | `Item source has no definition` | ❌ Falhou |
| 3 | Usar `$employee_id` (cifrão) | `<source><path>$employee_id</path></source>` | `Item source has no definition` | ❌ Falhou |
| 4 | Usar expressão (novo correlation engine) | `<ref>c:personalNumber</ref>` + `<expression><itemPath>ri:employee_id</itemPath></expression>` | `Item expression has no definition` | ❌ Falhou |
| 5 | Usar filter com q:equal | `<filter><q:equal>...` | `Item filter has no definition` | ❌ Falhou |

**Conclusão da investigação:** O CsvConnector v2.9 não suporta o novo Correlation Engine do midPoint 4.10. A abordagem testada e validada no **PRJ004** demonstrou que o mecanismo de **Projeção Direta** (confiar nos mapeamentos Strong + reação `Unmatched → Add focus`) é suficiente e funcional.
...
# Exemplo de saída: iga-project-midpoint-1   evolveum/midpoint:4.10
```
...
Versão: 1.3 (correções de segurança e rede)
"""
...
def sync_to_midpoint(filename):
    ...
    try:
        # SCP usando chave SSH configurada
        cmd = ["scp", "-o", "StrictHostKeyChecking=no", filename, f"{MIDPOINT_USER}@{MIDPOINT_HOST}:{MIDPOINT_PATH}"]
        subprocess.run(cmd, check=True)
        # Ajuste de permissão (se sudo NOPASSWD configurado)
        subprocess.run(["ssh", f"{MIDPOINT_USER}@{MIDPOINT_HOST}", f"sudo chmod 644 {MIDPOINT_PATH}"], check=True)
...
**Por que?** Este campo será a **chave de correlação** (implícita) para o midPoint.
...
## 12. Passo 6: Configuração da Correlation — ESTRATÉGIA VALIDADA (PRJ004)

### ⚠️ DECISÃO ARQUITETURAL — LEIA ATENTAMENTE

**O CsvConnector v2.9 no midPoint 4.10 NÃO SUPORTA o bloco `<correlation>` explícito.** Tentativas de configurar correlação via XML resultaram nos seguintes erros:

| Tentativa | Sintaxe | Erro |
|-----------|---------|------|
| Apenas `<ref>personalNumber</ref>` | `No correlation items` |
| `<source><path>ri:employee_id</path></source>` | `Item source has no definition` |
| `<source><path>$employee_id</path></source>` | `Item source has no definition` |
| `<expression><itemPath>ri:employee_id</itemPath></expression>` | `Item expression has no definition` |

**Portanto, NÃO CONFIGURE um bloco `<correlation>` explícito.** A estratégia validada no **PRJ004** é utilizar **Projeção Direta**: o midPoint infere automaticamente a correlação a partir dos mapeamentos **Strong** e da reação `Unmatched → Add focus`.

### 12.1. Verificar os Mapeamentos Strong (Já Configurados)

Os seguintes mapeamentos já foram configurados no Passo 5 como **Strong**:

| Atributo do CSV | Atributo do User | Strength |
|-----------------|------------------|----------|
| `employee_id` | `name` | **Strong** |
| `employee_id` | `personalNumber` | **Strong** |

### 12.2. NÃO ADICIONAR REGRA DE CORRELAÇÃO

- **Não clique em "Add rule"**
- **Não modifique o bloco Correlation**
- **Mantenha a seção Correlation vazia**

### 12.3. Por que isso funciona?

O midPoint, ao encontrar um objeto com atributo `personalNumber` mapeado como **Strong**, automaticamente utiliza este atributo como **chave de correlação primária**. O motor de sincronização busca por um usuário existente com o mesmo `personalNumber` antes de decidir criar um novo.

**Evidência empírica:** O PRJ004 processou 10 usuários com sucesso, sem correlation explícita, e sem criar duplicatas em múltiplas execuções.

### 12.4. Auditoria e Rastreabilidade

A ausência de um bloco `<correlation>` explícito **NÃO** compromete o rastro de auditoria (ISO 27001 A.8.15). O log de auditoria registra todas as operações com o mesmo nível de detalhe:

| Timestamp | Evento | Detalhe |
|-----------|--------|---------|
| T+0 | Correlation | personalNumber=XXXX not found |
| T+0 | Synchronization | Situation=Unmatched, Action=Add focus |
| T+1 | Repository | User created (OID: xxxxx) |
...
Clique em **Next: Schedule**

### 14.5. Tela 3 — Schedule

- **Não preencher nada** (execução manual imediata)

Clique em **Next: Distribution**

### 14.6. Tela 4 — Distribution

- **Não preencher nada** (usar uma única thread)
...
4. Verifique o contador de **Successes** — deve chegar a **102** quando concluído
...
| **Usuários duplicados** | Correlation não está funcionando | Verificar se `personalNumber` está como **Strong** no mapeamento |
| **Item source has no definition** | Tentativa de correlation explícita | **Remover** o bloco `<correlation>` do XML |
| **Item expression has no definition** | Tentativa de novo correlation engine | **Remover** o bloco `<correlation>` do XML |
| **API Key expirada** | Token do Vault foi revogado | Gerar novo token e atualizar variável de ambiente |
...
| L07 | O wizard de Object Type tem 8 blocos; Mappings e Synchronization são os essenciais |
| L08 | O mapeamento `employee_id` → `personalNumber` com **Strength Strong** serve como chave de correlação implícita |
| L09 | O botão "Save & Run" na tarefa salva **e** executa imediatamente |
| **L10** | **O CsvConnector v2.9 NÃO suporta o bloco `<correlation>` explícito** — usar Projeção Direta (PRJ004 style) |
| **L11** | Tentativas de usar `<source>`, `<expression>` ou `<filter>` resultam em erros de parser |
| **L12** | A correlação implícita baseada em mapeamentos **Strong** gera rastro de auditoria completo (ISO 27001 A.8.15) |

### 18.2. Tentativas e Erros Documentados (PRJ022)

| # | Tentativa | Sintaxe | Erro | Conclusão |
|---|-----------|---------|------|-----------|
| T01 | Via GUI (sem XML) | Apenas `<ref>personalNumber</ref>` | `No correlation items` | ❌ Inválida |
| T02 | Adicionar `<source>` | `<source><path>ri:employee_id</path></source>` | `Item source has no definition` | ❌ Inválida |
| T03 | Usar `$employee_id` | `<source><path>$employee_id</path></source>` | `Item source has no definition` | ❌ Inválida |
| T04 | Novo correlation engine | `<ref>c:personalNumber</ref>` + `<expression><itemPath>ri:employee_id</itemPath></expression>` | `Item expression has no definition` | ❌ Inválida |
| T05 | Usar filter | `<filter><q:equal>...` | `Item filter has no definition` | ❌ Inválida |
| **T06** | **Projeção Direta** | **Sem bloco `<correlation>`** | **✅ Sucesso** | **✅ Válida** |
...
| **ISO 27001** | A.8.15 (Log de Auditoria) | Transições de estado registradas mesmo sem correlation explícita |
...
sudo docker logs $(sudo docker ps -q --filter "name=midpoint") --tail 50
```
...
| **1.3** | **03/05/2026** | **Paulo Feitosa Lima** | **Nova estratégia de Correlation: Projeção Direta (baseada no PRJ004). Documentadas as tentativas fracassadas com `<source>`, `<expression>` e `<filter>`. Remoção do bloco `<correlation>` explícito. Lições L10-L12 adicionadas.** |
...
*PRJ022 — Procedimento Operacional Padrão (POP) v1.3*  
*Living Lab Fiqueok*  
*Arquivado em: `FiqueokBrain/PRJ022/POP-PRJ022-A-CSV-Integration-v1.3.md`*
